...
from sqlalchemy import create_engine, MetaData, Table, select, update, null
from sqlalchemy.orm import sessionmaker

DATABASE_URL = "postgresql://ctc_dev_06r2_user:c7jxzBBZTHTNeequSLRVLflYYhZo1Nq6@dpg-d1l00m15pdvs73bbq1og-a.oregon-postgres.render.com/ctc_dev_06r2"

engine = create_engine(DATABASE_URL)
SessionLocal = sessionmaker(autocommit=False, autoflush=False, bind=engine)

@router.post("/notification/clean_data_Base")
async def send_t(table_name: str):
    ...
    try:
        ...
        for row in results:
            ...
            for column in table.columns:
                ...
                print("123 : " ,column)
                print("1 : " ,colum_numbers)
                
                if row[colum_numbers] is None:
                    print("1yes 1 : ", row[colum_numbers])
                    print("1yes 1")
                    update_data[column.name] = ""
                colum_numbers += 1
                print("2 : " ,colum_numbers)

            print("updates : " ,colum_numbers)
            print("updat : " ,update_data)
            if update_data:
